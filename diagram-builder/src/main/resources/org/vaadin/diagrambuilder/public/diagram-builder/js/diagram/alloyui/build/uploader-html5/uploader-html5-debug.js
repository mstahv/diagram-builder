...
// Shorthands for the external modules
    ...
    Y.UploaderHTML5 = Y.extend(UploaderHTML5, Y.Widget, {
         ...
         * @param postVars {Object} (optional) A set of key-value pairs to send as variables along with the file upload
         *     HTTP request. If not specified, the values from the attribute `postVarsPerFile` are used instead.
         */
         ...
         * @param [postVars] {Object} A set of key-value pairs to send as variables along with the file upload HTTP
         *     request. If not specified, the values from the attribute `postVarsPerFile` are used instead.
         */
            ...
            if (!this.queue) {
                ...
                Y.each(files, function (file) {
                    this.queue.addToQueueBottom(file);
                                   ...
                                   /**
                                    * The template for the hidden file input field container. The file input field will
                                    * only accept clicks if its visibility is set to hidden (and will not if it's
                                    * `display` value is set to `none`)
                                    *
                                    ...
                                    * @type {String}
                                    * @static
                                    * @default '<button type="button" class="yui3-button" role="button"
