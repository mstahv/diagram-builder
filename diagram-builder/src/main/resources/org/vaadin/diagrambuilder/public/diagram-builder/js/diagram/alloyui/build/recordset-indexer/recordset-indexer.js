YUI.add('recordset-indexer', function (Y, NAME) {
     ...
     * This utility does not support any collision handling. New hash table entries with a used key overwrite older
     * ones.
    ...
    Y.extend(RecordsetIndexer, Y.Plugin.Base, {
            ...
            Y.each(tbl,
                   function (v, key) {
                       Y.each(e.added || e.updated,
                           function (o) {
                               //if the object being added has a key which is being stored by hashtable v, add it into
                               // the table.
                               if (o.getValue(key)) {
                                   v[o.getValue(key)] = o;
                               }
                   ...
                   function (v, key) {
                       Y.each(e.removed || e.overwritten,
                           function (o) {
                               ...
                               //if the hashtable has a key storing a record, and the key and the record both match the
                               // record being deleted, delete that row from the hashtable
        ...
        _defUpdateHash: function (e) {
            ...
            //TODO: It will be more performant to create a new method rather than using _defAddHash, _defRemoveHash,
            // due to the number of loops. See commented code.
            e.added = e.updated;
