    ...
    Y.CalendarBase = Y.extend(CalendarBase, Y.Widget, {
        ...
        _renderCustomRulesHelper: function (date) {
            ...
            if (matchingRules.length > 0) {
                if ((enRule && iOf(matchingRules, enRule) < 0) || (!enRule && disRule && iOf(matchingRules, disRule)
                                                                   >= 0)) {
        ...
        _initCalendarPane: function (baseDate, pane_id) {
            var dateFormat = Y.Intl.get('datatype-date-format'),
                ...
                // Compute the cutoff column of the masked calendar table, based on the start date and the first day of
                // week.
            ...
            for (row = 0; row <= 5; row++) {
                ...
                for (column = 0; column <= 12; column++) {
                    ...
                    // Decide on whether a column in the masked table is visible or not based on the value of the
                    // cutoff column.
                    column_visibility = (column >= cutoffCol && column < (cutoffCol + 7)) ? '' : CAL_COL_HIDDEN;

                    // Substitute the values into the partial calendar day template and add it to the current row HTML
                    // string
        ...
        _rerenderCalendarPane: function (newDate, pane) {
            ...
            // Go through all columns, and flip their visibility setting based on whether they are within the unmasked
            // range.
            for (column = 0; column <= 12; column++) {
        ...
        _initCalendarHTML: function (baseDate) {
            ...
            // Go through all occurrences of the calendar_grid_template token and replace it with an appropriate
            // calendar grid.
