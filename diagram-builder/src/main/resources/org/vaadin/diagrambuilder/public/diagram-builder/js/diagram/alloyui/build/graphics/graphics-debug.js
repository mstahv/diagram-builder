YUI.add('graphics', function (Y, NAME) {
     ...
     * <a href="http://www.w3.org/TR/html5/the-canvas-element.html">Canvas</a> and <a
     * href="http://www.w3.org/TR/NOTE-VML">VML</a> to render its graphical elements.</p>
     *
     * <p>The `Graphics` module features a <a href="../classes/Graphic.html">`Graphic`</a> class that allows you to
     * easily create and manage shapes. Currently, a <a href="../classes/Graphic.html">`Graphic`</a> instance can be
     * used to create predifined shapes and free-form polygons with fill and stroke properties.</p>
     *
...
//Straightup augment, no wrapper functions
     ...
     */
    /**
     * Draws a line segment using the current line style from the current drawing position to the specified x and y
     * coordinates.
     *
     * @method lineTo
     ...
     *  </ul>
     *
     * It is not necessary to interact with these classes directly. `Shape` will point to the appropriate
     * implemention.</p>
     *
     * @class Shape
     * @constructor
     ...
     *
     * Calculates the a new bounding box from the original corner coordinates (base on size and position) and the
     * transform matrix. The calculated bounding box is used by the graphic instance to calculate its viewBox.
     *
     * @method getBounds
     * @return Object
     */
     ...
     * between 0 and 1 representing a fraction of the shape's corresponding bounding box dimension. The default value
     * is [0.5, 0.5].
     *
     ...
     */
    /**
     * <p>A string containing, in order, transform operations applied to the shape instance. The `transform` string can
     ...
     * transform. The shape class also contains corresponding methods for each transform that will apply the transform
     * to the current matrix. The below code illustrates how you might use the
     ...
     *      <dt>rotation</dt><dd>Linear gradients flow left to right by default. The rotation property allows you to
     * change the flow by rotation. (e.g. A rotation of 180 would make the gradient pain from right to left.)</dd>
     *      <p>Radial gradients have the following additional properties:</p>
     *      <dt>r</dt><dd>Radius of the gradient circle.</dd>
     *      <dt>fx</dt><dd>Focal point x-coordinate of the gradient.</dd>
     *      <dt>fy</dt><dd>Focal point y-coordinate of the gradient.</dd>
     *      <dt>cx</dt><dd>
     *          <p>The x-coordinate of the center of the gradient circle. Determines where the color stop begins. The
     * default value 0.5.</p>
     *          <p><strong>Note: </strong>Currently, this property is not implemented for corresponding `CanvasShape`
     * and
     *          `VMLShape` classes which are used on Android or IE 6 - 8.</p>
     *      </dd>
     *      <dt>cy</dt><dd>
     *          <p>The y-coordinate of the center of the gradient circle. Determines where the color stop begins. The
     * default value 0.5.</p>
     *          <p><strong>Note: </strong>Currently, this property is not implemented for corresponding `CanvasShape`
     * and `VMLShape` classes which are used on Android or IE 6 - 8.</p>
     *      </dd>
     *  </dl>
     *
     * @config fill
     ...
     *      <dt>opacity</dt><dd>Number between 0 and 1 that indicates the opacity of the stroke. The default value is
     * 1.</dd>
     *      <dt>dashstyle</dt>Indicates whether to draw a dashed stroke. When set to "none", a solid stroke is drawn.
     * When set to an array, the first index indicates the length of the dash. The second index indicates the length of
     * gap.
     ...
     *              <dt>miter limit</dt><dd>An integer specifying the miter limit of a miter linejoin. If you want to
     * specify a linejoin of miter, you simply specify the limit as opposed to having separate miter and miter limit
     * values.</dd>
     *          </dl>
     *      </dd>
     *  </dl>
     ...
     * implementations. Note that when using VML or SVG implementations, part of this content will be added to the DOM
     * using respective VML/SVG attributes. If your content comes from an untrusted source, you will need to ensure
     ...
     * <p>`Circle` instances can be created using the <a href="Graphic.html#method_addShape">`addShape`</a> method of
     * the
     * <a href="Graphic.html">`Graphic`</a> class. The method's `cfg` argument contains a `type` attribute. Assigning
     * "circle" or `Y.Circle` to this attribute will create a `Circle` instance. Required attributes for instantiating
     * a `Circle` are
     ...
     *  </ul>
     *
     * It is not necessary to interact with these classes directly. `Circle` will point to the appropriate
     * implemention.</p>
     *
     * @class Circle
     * @extends Shape
     ...
     * <p>`Ellipse` instances can be created using the <a href="Graphic.html#method_addShape">`addShape`</a> method of
     * the
     * <a href="Graphic.html">`Graphic`</a> class. The method's `cfg` argument contains a `type` attribute. Assigning
     * "ellipse" or `Y.Ellipse` to this attribute will create a `Ellipse` instance. Required attributes for
     * instantiating a `Ellipse` are
     ...
     *  </ul>
     *
     * It is not necessary to interact with these classes directly. `Ellipse` will point to the appropriate
     * implemention.</p>
     *
     * @class Ellipse
     * @extends Shape
     ...
     * <a href="Graphic.html">`Graphic`</a> class. The method's `cfg` argument contains a `type` attribute. Assigning
     * "rect" or `Y.Rect` to this attribute will create a `Rect` instance. Required attributes for instantiating a
     * `Rect` are `type`,
     ...
     *  </ul>
     *
     * It is not necessary to interact with these classes directly. `Rect` will point to the appropriate
     * implemention.</p>
     *
     * @class Rect
     * @extends Shape
     ...
     */
    /**
     * <p>The `Path` class creates a shape through the use of drawing methods. The `Path` class has the following
     ...
     *  </ul>
     *
     *  <p>Like other shapes, `Path` elements are created using the <a
     * href="Graphic.html#method_addShape">`addShape`</a> method of the <a href="Graphic.html">`Graphic`</a> class. The
     * method's `cfg` argument contains a `type` attribute. Assigning "path" or `Y.Path` to this attribute will create
     * a `Path` instance. After instantiation, a series of drawing operations must be performed in order to render a
     * shape. The below code instantiates a path element by defining the
     ...
     * Below a `Path` element with the same properties is instantiated by defining the `type` attribute with a class
     * reference:
     *
     var myPath = myGraphic.addShape({
            type: Y.Path,
     ...
     * After instantiation, a shape or segment needs to be drawn for an element to render. After all draw operations
     * are performed, the <a href="#method_end">`end`</a> method will render the shape. The code below will draw a
     * triangle:
     myPath.moveTo(35, 5);
     myPath.lineTo(65, 65);
     myPath.lineTo(5, 65);
     ...
     *  </ul>
     * It is not necessary to interact with these classes directly. `Path` will point to the appropriate
     * implemention.</p>
     *
     * @class Path
     * @extends Shape
     ...
     */
    /**
     * `Graphic` acts a factory and container for shapes. You need at least one `Graphic` instance to create shapes for
     * your application.
     * <p>The code block below creates a `Graphic` instance and appends it to an HTMLElement with the id
     * 'mygraphiccontainer'.</p>
     var myGraphic = new Y.Graphic({render:"#mygraphiccontainer"});
     * <p>Alternatively, you can add a `Graphic` instance to the DOM using the <a href="#method_render">`render`</a>
     * method.</p>
     ...
     *  </ul>
     *
     * It is not necessary to interact with these classes directly. `Graphic` will point to the appropriate
     * implemention.</p>
     *
     * @class Graphic
     * @constructor
     */
    /**
     * Whether or not to render the `Graphic` automatically after to a specified parent node after init. This can be a
     * Node instance or a CSS selector string.
     *
     * @config render
     * @type Node | String
     */
     ...
     * element. The content contained in the Graphic will be sized to fit with in the Graphic instance's dimensions.
     * When using this setting, the <code>preserveAspectRatio</code> attribute will determine how the contents are
     * sized.</dd>
     ...
     * <code>width</code> and <code>height</code> attributes or are determined by the dimensions of the parent element.
     * The contents of the Graphic instance are not affected by this setting.</dd>
     *  </dl>
     *
     *
     *  @config autoSize
     ...
     *
     *  <dl>
     *      <dt>none<dt><dd>Do not force uniform scaling. Scale the graphic content of the given element non-uniformly
     * if necessary such that the element's bounding box exactly matches the viewport rectangle.</dd>
     *      <dt>xMinYMin</dt><dd>Force uniform scaling position along the top left of the Graphic's node.</dd>
     *      <dt>xMidYMin</dt><dd>Force uniform scaling horizontally centered and positioned at the top of the Graphic's
     * node.<dd>
     *      <dt>xMaxYMin</dt><dd>Force uniform scaling positioned horizontally from the right and vertically from the
     * top.</dd>
     ...
     * centered.</dd>
     *      <dt>xMinYMax</dt><dd>Force uniform scaling positioned horizontally from the left and vertically from the
     * bottom.</dd>
     *      <dt>xMidYMax</dt><dd>Force uniform scaling horizontally centered and position vertically from the
     * bottom.</dd>
     *      <dt>xMaxYMax</dt><dd>Force uniform scaling positioned horizontally from the right and vertically from the
     * bottom.</dd>
     *  </dl>
     *
