YUI.add('axis-numeric-base', function (Y, NAME) {
    ...
    NumericImpl.ATTRS = {
        ...
        /**
         * Method used for formatting a label. This attribute allows for the default label formatting method to
         * overridden. The method use would need to implement the arguments below and return a `String` or an
         * `HTMLElement`. The default implementation of the method returns a `String`. The output of this method will
         * be rendered to the DOM using
         * `appendChild`. If you override the `labelFunction` method and return an html string, you will also need to
         * override the Data' `appendLabelFunction` to accept html as a `String`.
         * <dl>
                    ...
                    if (minGreaterThanZero && maxGreaterThanZero) {
                        ...
                        if (alwaysShowZero) {
                            ...
                            if (setMin) {
                                ...
                                //if there are any bottom ticks left calcualate the maximum by multiplying by the
                                // tempMin value if not, it's impossible to ensure that a zero is shown. skip it
                                if (botTicks > 0) {
                                ...
                                //if there are any top ticks left calcualate the minimum by multiplying by the tempMax
                                // value if not, it's impossible to ensure that a zero is shown. skip it
                                if (topTicks > 0) {
