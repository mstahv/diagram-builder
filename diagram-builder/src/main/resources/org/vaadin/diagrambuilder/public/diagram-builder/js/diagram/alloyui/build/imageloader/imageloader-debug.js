YUI.add('imageloader', function (Y, NAME) {
    ...
    /**
     * A group for images. A group can have one time limit and a series of triggers. Thus the images belonging to this
     * group must share these constraints.
     * @class ImgLoadGroup
     * @extends Base
     * @constructor
     */
    ...
    Y.ImgLoadGroup.ATTRS = {
        ...
        /**
         * Distance below the fold for which images are loaded. Images are not loaded until they are at most this
         * distance away from (or above) the fold. This check is performed at page load (domready) and after any window
         * scroll or window resize event (until all images are loaded).
        ...
        /**
         * Class name that will identify images belonging to the group. This class name will be removed from each
         * element in order to fetch images. This class should have, in its CSS style definition,
         * "<code>background:none !important;</code>".
         * @attribute className
         * @type String
         */
        className: {
    ...
    var groupProto = {
        ...
        _init: function () {
             ...
             * Elements are stored during the <code>_foldCheck</code> function and reused later during any subsequent
             * <code>_foldCheck</code> calls - gives a slight performance improvement when the page fold is repeatedly
             * checked.
            ...
            /**
             * Boolean tracking whether the window scroll and window resize triggers have been set if this is a fold
             * group.
         ...
         * @param {Object} obj  The object on which to attach the event. <code>obj</code> is optional - by default the
         *     event is attached to the <code>Y</code> instance
         */
        ...
        /**
         * Sets the window scroll and window resize triggers for any group that is fold-conditional (i.e., has a fold
         * distance set).
        ...
        _onloadTasks: function () {
            ...
            if (timeLim && timeLim > 0) {
                Y.log('setting time limit of ' + timeLim + ' seconds for group: ' + this.get('name'), 'info',
                                                                                                      'imageloader');
         ...
         * Arguments are passed through to a <code>Y.ImgLoadImgObj</code> constructor; see that class' attribute
         * documentation for detailed information. "<code>domId</code>" is a required attribute.
         * @method registerImage
         * @param {Object} config A configuration object literal with attribute name/value pairs  (passed through to a
         *     <code>Y.ImgLoadImgObj</code> constructor)
         ...
         * This method is called when a trigger fires or the time limit expires; it shouldn't be called externally, but
        ...
        /**
         * Checks the position of each image in the group. If any part of the image is within the specified distance
         * (<code>foldDistance</code>) of the client viewport, the image is fetched immediately.
         * @method _foldCheck
         * @private
         */
        _foldCheck: function () {
            ...
            if (this._className) {
                ...
                for (i = 0, len = els.length; i < len; i++) {
                    ...
                    if (els[i].y && els[i].y <= hLimit) {
                        ...
                        els[i].fetched = true;
                        Y.log('Image with id "' + els[i].el.get('id')
                              + '" is within distance of the fold. Fetching image. (Image registered by class name with the group - may not have an id.)',
                            'info', 'imageloader');
                    }
                    else {
                        allFetched = false;
            ...
            if (allFetched) {
                Y.log('All images fetched; removing listeners for group: "' + this.get('name') + '"', 'info',
                                                                                                      'imageloader');
                this._clearTriggers();
            }
        },
        ...
        /**
         * Finds all elements in the DOM with the class name specified in the group. Removes the class from the element
         * in order to let the style definitions trigger the image fetching.
        ...
        _fetchByClass: function () {
            ...
            Y.log('Fetching all images with class "' + this._className + '" in group "' + this.get('name') + '".',
                'info', 'imageloader');
    ...
    Y.ImgLoadImgObj.ATTRS = {
        ...
        /**
         * Pixel width of the image. Will be set as a <code>width</code> attribute on the DOM element after the image
         * is fetched. Defaults to the natural width of the image (no <code>width</code> attribute will be set).
        ...
        /**
         * Pixel height of the image. Will be set as a <code>height</code> attribute on the DOM element after the image
         * is fetched. Defaults to the natural height of the image (no <code>height</code> attribute will be set).
         * Usually only used with src images.
         * @attribute height
         * @type Int
         */
        ...
        /**
         * Whether the image's <code>style.visibility</code> should be set to <code>visible</code> after the image is
         * fetched. Used when setting images as <code>visibility:hidden</code> prior to image fetching.
         ...
         * PNG images get special treatment in that the URL is specified through AlphaImageLoader for IE, versions 6
         * and earlier. Only used with background images.
         * @attribute isPng
         * @type Boolean
         */
        isPng: {
    ...
    var imgProto = {
        ...
        _init: function () {
             ...
             * The Y position is checked only for images registered with fold-conditional groups. The position is
             * checked first at page load (domready) and this caching enhancement assumes that the image's vertical
             * position won't change after that first check.
             * @property _yPos
             * @private
             * @type Int
             */
         ...
         * This method shouldn't be called externally, but is not private in the rare event that it needs to be called
         * immediately.
         * @method fetch
         * @param {Number} withinY  The pixel distance from the top of the page, for which if the image lies within, it
         *     will be fetched. Undefined indicates that no check should be made, and the image should always be
         *     fetched
        ...
        fetch: function (withinY) {
            ...
            if (withinY) {
                ...
                Y.log('Image with id "' + this.get('domId') + '" is within distance of the fold. Fetching image.',
                    'info', 'imageloader');
            }
