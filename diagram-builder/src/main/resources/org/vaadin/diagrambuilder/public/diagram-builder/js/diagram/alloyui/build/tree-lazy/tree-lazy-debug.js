    ...
    Y.namespace('Plugin.Tree').Lazy = Y.Base.create('lazyTreePlugin', Y.Plugin.Base, [], {
        ...
        initializer: function (config) {
            ...
            if (!this._host.openNode) {
                Y.log(
                    "Plugin.Tree.Lazy was plugged into a Tree that doesn't mix in the Tree.Openable extension. This probably won't do you much good.",
                    'warn', 'tree-lazy');
            }
        ...
        load: function (node, callback) {
            callback(
                new Error('Plugin.Tree.Lazy: Please provide a custom `load` method when instantiating this plugin.'));
        },
