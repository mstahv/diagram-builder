...
// Just a simple namespace to make methods overridable.
        VC = {
            ...
            _poll: function (node, options) {
                ...
                if (!(domNode && vcData)) {
                    Y.log('_poll: node #' + node.get('id')
                          + ' disappeared; stopping polling and removing all notifiers.', 'warn', 'event-valuechange');
            ...
            _startPolling: function (node, notifier, options) {
                ...
                if (!node.test('input,textarea,select') && !(isEditable = VC._isEditable(node))) {
                    Y.log('_startPolling: aborting poll on #' + node.get('id') + ' -- not a detectable node', 'warn',
                                                                                                              'event-valuechange');
                    return;
                }
