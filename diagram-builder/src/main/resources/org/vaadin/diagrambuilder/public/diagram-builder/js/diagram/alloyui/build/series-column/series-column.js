YUI.add('series-column', function (Y, NAME) {
    ...
    Y.ColumnSeries = Y.Base.create("columnSeries", Y.MarkerSeries, [Y.Histogram], {
         ...
         * @param {Number} calculatedSize The calculated size for the marker. For a `BarSeries` is it the width. For a
         *     `ColumnSeries` it is the height.
        ...
        updateMarkerState: function (type, i) {
            if (this._markers && this._markers[i]) {
                var styles = this._copyObject(this.get("styles").marker),
                    ...
                    config;
                markerStyles =
                    state === "off" || !styles[state] ? this._copyObject(styles) : this._copyObject(styles[state]);
                        ...
                        if (renderer && renderer !== undefined) {
                            renderer.set("x", (xs[n] - seriesSize / 2));
                                       ...
                                       ATTRS: {
                                           ...
                                           /**
                                            * Style properties used for drawing markers. This attribute is inherited
                                            * from `MarkerSeries`. Below are the default values:
                                            *  <dl>
                                            ...
                                            * by a `mouseover` event. The default values for each style is null. When
                                            * an over style is not set, the non-over value will be used. For example,
                                            * the default value for `marker.over.fill.color` is equivalent to
